...
void matrix_recounstruction(const double *R,double *P,double *Q, int* PU, int N,int M,int K,int steps=STEPR,float alpha=ALAPA,float beta=BETA) {
...
int main() {
    ...
    while(gg++<N*K/2) {
        ...
        for(int i=0; i<N; i++)
            for(int j=0; j<K; j++) {
                P2[i*K+j] = P[i*K+j];
                if(!PU[i*K+j] && max<abs(P[i*K+j])) {// 寻找P中未重复出现过的最小值位置
                    max=P[i*K+j];
                    x=i;
                    y=j;
                }
        ...
        P2[x*K+y] = PMAX;
        PU[x*K+y] = 1;
        matrix_recounstruction(R,P2,Q2,PU,N,M,K); // 将修改后的P2代入去迭代
        if(cal_rmse(R,P2,Q,N,M,K)<lossr*1.2) { // 观察rmse变化情况，若增加太多则不改变
