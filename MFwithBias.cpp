...
double cal_rmse(const double *R,const double *B,const double *P,const double *Q,const int N,const int M,const int K,const int L) {
    double loss = 0;
    for(int i=0; i<N; ++i) {
        for(int j=0; j<M; ++j) {
            if(R[i*M+j] > 0 ) {
                ...
                if(R[i*M+j]>0) {
                    ...
                    for(int k=0; k<K; ++k) {
                        ...
                        /*
                            ...
                            Q(i,j) += alpha * e(i,j) * (P(k,j)+B[i]) - beta * Q(i,j)
                        */
                    }
                }
            }
