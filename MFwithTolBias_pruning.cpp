...
double cal_error(const double *R,const double *B,const double *C,const double *P,const double *Q,int N,int M,int K,int V) {
    double loss = 0;
    for(int j=0; j<M; ++j) {
        if(R[V*M+j] > 0 ) {
...
void vector_reconstruction(const double *R,double *B,double *C,double *P,double *Q,int N,int M,int K,int V,int steps=STEPR,float alpha=ALAPA,float beta=BETA) {
    double error;
    for(int step =0; step<steps; ++step) { // 重建与初次的差异步骤不大，但需要判断为0的位置不进行更新
        for(int j=0; j<M; ++j) {
            if(R[V*M+j]>0) {
...
void print_rst(double* T, double* B, double* C, double* P, double* Q,int N,int M,int K,int Tlen) {
    double loss = 0, loss1 = 0, lossabs1 = 0, error;
    for(int i=0; i<N; ++i) {
        for(int j=0; j<M; ++j) {
            if(T[i*M+j] > 0 ) {
                error=B[i] + C[j];
...
//  ----------------------------------------------Purning------------------------------------------------------------
    ...
    while(gg++<N*K) {
        ...
        vector_reconstruction(R,B2,C2,P2,Q2,N,M,K,x/K);
